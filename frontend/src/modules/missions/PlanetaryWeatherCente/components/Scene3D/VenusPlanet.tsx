...
export const VenusPlanet: React.FC<VenusPlanetProps> = ({ activeEvent }) => {
  ...
  // Текстуры (временные, пока нет настоящих)
  ...
  useFrame(() => {
    ...
    if (cloudLayerRef.current) {
      // Облака вращаются быстрее поверхности
  ...
  return (
    <>
      {/* Поверхность Венеры (едва видна под облаками) */}
      <mesh ref={planetRef}>
      ...
      {/* Верхний слой облаков — кислотная дымка */}
      <mesh>
      ...
      {/* Атмосферные эффекты (вспышки молний внутри облаков) */}
      <VenusAtmosphere 
