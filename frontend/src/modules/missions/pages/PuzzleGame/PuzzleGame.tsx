...
interface LevelConfig {
  ...
  pieces: number;
  timeLimit: number;
  starThresholds: [number, number, number]; // время для 3, 2, 1 звёзд (в секундах)
}
...
const LEVELS: LevelConfig[] = [
  ...
  { name: 'СЛОЖНЫЙ', cols: 8, rows: 8, pieces: 64, timeLimit: 420, starThresholds: [120, 240, 420] },
  { name: 'ЭКСТРЕМАЛЬНЫЙ', cols: 16, rows: 8, pieces: 128, timeLimit: 600, starThresholds: [180, 360, 600] },
];
...
const generatePieces = (satellite: SatelliteInfo, level: LevelConfig, containerWidth: number, containerHeight: number): PuzzlePiece[] => {
  ...
  const pieceW = Math.min(100, (containerWidth - 40) / level.cols);
  const pieceH = Math.min(100, (containerHeight - 100) / level.rows);
  const totalW = level.cols * pieceW;
  const totalH = level.rows * pieceH;
  const offsetX = (containerWidth - totalW) / 2;
  const offsetY = (containerHeight - totalH) / 2 + 40;
  ...
  for (let i = 0; i < count; i++) {
    ...
    pieces.push({
      id: `piece-${i}`,
      x: Math.random() * (containerWidth - pieceW - 60) + 30,
      y: Math.random() * (containerHeight - pieceH - 100) + 60,
...
export const PuzzleGame: React.FC<PuzzleGameProps> = ({ onClose, onWin }) => {
  ...
  const startGame = useCallback(() => {
    ...
    const h = puzzleAreaRef.current?.clientHeight || window.innerHeight * 0.75;
  ...
  useEffect(() => {
    ...
    const u = () => {
      ...
      setPieces(prev => {
        ...
        const snap = dist < 40;
        const upd = prev.map(p => p.id === dragging ? { ...p, x: snap ? p.targetX : p.x, y: snap ? p.targetY : p.y, placed: p.placed || snap } : p);
        if (upd.every(p => p.placed)) {
          ...
          setLevelProgress(prev => ({
            ...
            [selectedSatellite.id]: {
              ...prev[selectedSatellite.id],
              [selectedLevel.name]: {
                stars: Math.max(stars, prev[selectedSatellite.id]?.[selectedLevel.name]?.stars || 0),
                bestTime: prev[selectedSatellite.id]?.[selectedLevel.name]?.bestTime
                  ? Math.min(elapsedTime, prev[selectedSatellite.id][selectedLevel.name].bestTime!)
                  : elapsedTime,
              },
            },
          }));
  ...
  const getPieceStyle = (piece: PuzzlePiece) => {
    const col = piece.imageIndex % selectedLevel.cols;
    const row = Math.floor(piece.imageIndex / selectedLevel.cols);
    const pieceW = Math.min(100, ((puzzleAreaRef.current?.clientWidth || 800) - 40) / selectedLevel.cols);
    const pieceH = Math.min(100, ((puzzleAreaRef.current?.clientHeight || 600) - 100) / selectedLevel.rows);
      ...
      <div ref={windowRef} className={`${styles.container} ${isMaximized ? styles.fullscreen : ''}`}
        ...
        <div className={styles.titleBar} onMouseDown={handleTitleMouseDown}>
          <div className={styles.titleLeft}>
            <span className={styles.titleText}>Спутниковый конструктор{screen === 'game' ? ` — ${selectedSatellite.name} (${selectedLevel.name})` : ''}</span>
          </div>
          ...
          {screen === 'intro3' && (
            <div className={styles.introScreen}>
              ...
              <p className={styles.introText}>
                От 4 до 128 деталей. Собирай быстрее —<br/>получай больше звёзд!
              </p>
              ...
              <div className={styles.satelliteInfoRight}>
                ...
                <div className={styles.levelGrid}>
                  {LEVELS.map(level => {
                    ...
                    return (
                      <div key={level.name} className={styles.levelCard} onClick={() => { setSelectedLevel(level); startGame(); }}>
            ...
            <>
              ...
              <div className={styles.puzzleArea} ref={puzzleAreaRef}>
                {pieces.filter(p => !p.placed).map(p => (
                  <div key={`t-${p.id}`} className={styles.targetSpot} style={{ left: p.targetX, top: p.targetY, width: getPieceStyle(p).width, height: getPieceStyle(p).height }} />
                ))}
                {pieces.filter(p => p.placed).map(p => (
                  <div key={`pl-${p.id}`} className={`${styles.piece} ${styles.placed}`} style={{ left: p.targetX, top: p.targetY, ...getPieceStyle(p) }} />
                ))}
                {pieces.filter(p => !p.placed).map(p => (
                  <div key={p.id} className={`${styles.piece} ${dragging === p.id ? styles.dragging : ''}`} style={{ left: p.x, top: p.y, ...getPieceStyle(p) }} onMouseDown={e => handleMouseDown(e, p.id)} />
                ))}
              </div>
            </>
          )}
          ...
          {gameOver && screen === 'game' && (
            <div className={styles.gameOverlay}>
              ...
              <div className={styles.gameOverButtons}>
                <button onClick={() => { setSelectedLevel(selectedLevel); startGame(); }}>ПОВТОРИТЬ</button>
